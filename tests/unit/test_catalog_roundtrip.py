...
class TestCatalogRoundtrip(unittest.TestCase):
    ...
    def test_catalog_write_read_roundtrip(self):
        ...
        self.assertEqual(len(cat_in), n_events, 
        ...
        self._assert_events_equal(ev_map_in['event_000'], ev_map_out['event_000'])
        self._assert_events_equal(ev_map_in['event_001'], ev_map_out['event_001'])
        self._assert_events_equal(ev_map_in['event_002'], ev_map_out['event_002'])
        self._assert_events_equal(ev_map_in['event_003'], ev_map_out['event_003'])
        self._assert_events_equal(ev_map_in['event_004'], ev_map_out['event_004'])

    def test_catalog_preserves_all_fdsn_columns(self):
        """
        Verify that all 13 FDSN schema columns are present after a round-trip.

        The FDSN text format includes: evid, orig_time, lat, lon, depth, author,
        catalog, contributor, contributor_id, mag_type, mag, mag_author,
        location_name.
        """
